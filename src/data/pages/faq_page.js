export default {
    banner_title: "Preguntas Frecuentes",
    banner_subtitle: "Resolvemos tus dudas para que tu estadía sea sin sorpresas",
    banner_img: "/img/reception3.jpeg",
    preguntas: [
        {
            pregunta: "¿Dónde está ubicado el hotel?",
            respuesta: "Costado Norte de la Iglesia Los Dolores, Centro de Tegucigalpa.",
        },
        {
            pregunta: "¿Tiene servicio de comida?",
            respuesta: "Por el momento no, pero contamos con un área donde se venden snacks.",
        },
        {
            pregunta: "¿Cuál es el método de pago?",
            respuesta: "Puedes pagar en efectivo o con tarjeta.",
        },
        {
            pregunta: "¿Tiene aire acondicionado?",
            respuesta: "Sí, contamos con aire acondicionado y servicio de internet en todas las habitaciones.",
        },
        {
            pregunta: "¿Cuál es el horario de check-in y check-out?",
            respuesta: "Contamos con horarios flexibles a como guste el cliente y según cuántos días se quede. Además, ofrecemos tarifas por horas si necesitas mayor flexibilidad.",
        },
        {
            pregunta: "¿Se permite el ingreso de mascotas?",
            respuesta: "Por el momento no contamos con servicio para mascotas. Si tienes un caso especial, puedes consultarnos directamente al hotel.",
        },
    ],
    contacto: {
        direccion: "Costado Norte de la Iglesia Los Dolores, Centro de Tegucigalpa",
        telefono1: "9733-5556",
        telefono2: "8973-7935",
    },
};